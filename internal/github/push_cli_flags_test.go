...
func TestResolveCurrentPRHead_NotInRange(t *testing.T) {
	...
	sha, err := resolveCurrentPRHead(ChangeID{Number: 5}, false, false)
	if err != nil {
...
func TestResolveCurrentPRHead_InRange(t *testing.T) {
	...
	sha, err := resolveCurrentPRHead(ChangeID{Number: 5}, true, false)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if sha != "abc123" {
...
func TestResolveCurrentPRHead_FetchError(t *testing.T) {
	...
	if _, err := resolveCurrentPRHead(ChangeID{Number: 5}, true, false); err == nil {
	...
	stderr := captureStderr(t, func() {
		var err error
		sha, err = resolveCurrentPRHead(ChangeID{Number: 5}, true, true)
		if err != nil {
...
func TestLoadPushReview(t *testing.T) {
	t.Run("session and output conflict", func(t *testing.T) {
		_, _, err := loadPushReview(pushFlags{sessionID: "aaaaaaaaaaaa", outputDir: "/tmp/out"}, ChangeID{Number: 1})
	...
	t.Run("invalid session id", func(t *testing.T) {
		_, _, err := loadPushReview(pushFlags{sessionID: "bad"}, ChangeID{Number: 1})
	...
	t.Run("missing review file", func(t *testing.T) {
		...
		t.Chdir(projectDir)
		_, _, err := loadPushReview(pushFlags{}, ChangeID{Number: 1})
	...
	t.Run("reads existing review", func(t *testing.T) {
		...
		gotPath, cj, err := loadPushReview(pushFlags{}, ChangeID{Number: 0})
		if err != nil {
			t.Fatal(err)
		}
		if gotPath != identity {
	...
	t.Run("invalid review json", func(t *testing.T) {
		...
		_, _, err = loadPushReview(pushFlags{}, ChangeID{Number: 0})
	...
	t.Run("live session path", func(t *testing.T) {
		...
		gotPath, cj, err := loadPushReview(pushFlags{sessionID: key}, ChangeID{Number: 99})
		if err != nil {
			t.Fatal(err)
		}
		if gotPath != identity {
