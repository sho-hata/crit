...
type prMetadataCache struct {
	...
	fetchFn func(ChangeID) (*PRInfo, error)
}
...
func newPRMetadataCache() *prMetadataCache {
	return &prMetadataCache{
		...
		fetchFn: func(id ChangeID) (*PRInfo, error) { return fetchPRFn(id) },
	}
}
...
func (c *prMetadataCache) get(id ChangeID) (*PRInfo, error) {
	key := prCacheKey(id)
	c.mu.Lock()
...
func (c *prMetadataCache) invalidate(id ChangeID) {
	c.mu.Lock()
