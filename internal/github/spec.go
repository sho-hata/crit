...
func ParsePRSpec(spec string) (ChangeID, error) {
	if n, err := strconv.Atoi(spec); err == nil && n > 0 {
		return ChangeID{Number: n}, nil
	}
	u, err := url.Parse(spec)
	if err != nil || u.Hostname() == "" {
		return ChangeID{}, invalidPRSpec(spec)
	}
...
func prRepoRef(id ChangeID) string {
	if id.Project == "" {
		return ""
	}
...
func prViewArgs(id ChangeID) []string {
...
func prCacheKey(id ChangeID) string {
	if repo := prRepoRef(id); repo != "" {
...
func ghAPIArgs(id ChangeID, endpointAndFlags ...string) []string {
	args := []string{"api"}
	if repo := prRepoRef(id); repo != "" {
		args = append(args, "-R", repo)
	}
...
func resolveRepoOwnerName(id ChangeID) (string, string, error) {
	if id.Project != "" {
