...
func TestPRCacheKey_IncludesEnterpriseHost(t *testing.T) {
	got := prCacheKey(ChangeID{Number: 9, Project: "acme/app", Host: "github.example.com"})
	if got != "github.example.com/acme/app#9" {
		t.Fatalf("got %q", got)
	}
	dotcom := prCacheKey(ChangeID{Number: 9, Project: "acme/app", Host: "github.com"})
...
func TestGHAPIArgs(t *testing.T) {
	bare := ghAPIArgs(ChangeID{Number: 5}, "repos/{owner}/{repo}/pulls/5/comments")
	if !reflect.DeepEqual(bare, []string{"api", "repos/{owner}/{repo}/pulls/5/comments"}) {
		t.Fatalf("bare = %#v", bare)
	}
	pinned := ghAPIArgs(ChangeID{Number: 5, Project: "acme/widget"}, "repos/{owner}/{repo}/pulls/5/comments")
