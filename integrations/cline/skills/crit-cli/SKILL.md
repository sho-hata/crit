---
name: crit-cli
description: Use when an agent needs to author or reply to crit inline comments programmatically (including multi-agent workflows commenting on shared code/plans/docs/proposals), sync a crit review to or from a GitHub PR, or read/interpret a crit review JSON file. Covers crit comment, crit pull, crit push, review file format, and resolution workflow. Not for invoking an interactive review loop — that's the `/crit.md` workflow.
---

# Crit CLI Reference

> If a plan was just written and the user said `/crit.md` or `crit`, invoke the `/crit.md` workflow — do not use this reference skill. This skill covers CLI operations like `crit comment` and `crit pull/push`.
