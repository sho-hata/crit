...
> If the user invokes Crit by name (for example, "use Crit" or `/crit`), use the `/crit` skill instead — it covers the full review loop. This skill covers CLI operations like `crit comment` and `crit pull/push`.
