...
1. Install the language server(s) for the languages you review and make sure they are on your `PATH`:

   ```bash
   # Go — gopls also needs the go toolchain on PATH
...
3. To verify it's active:
   ...
   If it reports `false`: no supported language server is on the PATH crit was started from, `lsp` is disabled in config, the session has no repo root (plain files mode), or the review is a range/PR review that can't be backed by a local git checkout (`--remote`, or a Sapling/JJ repo — see below). `lsp_extensions` in the same response lists which file extensions are covered by the servers crit found.
...
- Definition and reference targets are only read from your repo and each installed language's known source roots: `GOROOT` and `GOMODCACHE` for Go, the global `node_modules` (`npm root -g`) for TypeScript, and for Python the stdlib and site-packages of the `python3` on your `PATH`, the project's `.venv`, and pyright's bundled typeshed. There is deliberately no general file-read endpoint. (TypeScript definitions into a locally-installed `node_modules` live under your repo root; definitions into a globally-installed `typescript` — e.g. `lib.dom.d.ts` after the global install above — are covered by the global-`node_modules` root as long as `npm` is on PATH.)
- **Range / PR reviews** (`--range`, `--pr`) show file content at a fixed SHA, which can differ from what's on disk. Rather than answering from the working tree and silently mismatching the diff you're reading, crit anchors the language servers at a sparse git worktree checked out at that SHA — built on the first LSP request and torn down on the same idle timeout. Local git only; `--remote` and Sapling/JJ range reviews report `lsp_available: false`. The checkout contains only the languages actually present in the review — a Go-only review doesn't pay for tracked JS assets just because typescript-language-server is installed. Cap the checkout size with `lsp_worktree_max_mb` (default 500) — a commit whose estimated checkout exceeds it is skipped rather than checked out. For TypeScript, `node_modules` is not part of the checkout (it isn't tracked by git), so cross-package hover/definition degrades in range/PR focus while intra-repo symbols keep working. A Python `.venv` is untracked too, so crit reads it from your working tree at the same path: third-party packages resolve against your *local* install, not the PR's dependencies (a bumped version shows the old API, a newly added dependency shows `Unknown`). Python hover tooltips in range/PR reviews, and peeks into a local package, carry a one-line note saying so; it is absent in working-tree reviews and for the stdlib.
- **Python virtualenvs.** pyright does not find a `.venv` by itself: without help every third-party import is `Unknown` — no type on hover, a definition that lands on the `import` line — and nothing reports an error. crit points pyright at the site-packages of the `.venv/` or `venv/` nearest the file, so a monorepo's per-service venvs work. It hands pyright a directory, never an interpreter: crit runs no Python from the repository under review, so opening a diff cannot execute code the branch brought along. Setups outside that convention are handled by pyright itself: activate the venv before starting crit (pyright follows the `python` on `PATH`; an already-running review keeps the `PATH` it started with, so restart it), or give the project a `pyrightconfig.json` with `venvPath` and `venv`. What only the interpreter knows — `.pth` files from editable installs, a Python version other than the one on `PATH` — is not read from a `.venv` this way.
- **Debugging.** Start crit with `CRIT_LSP_DEBUG=1` to log what crit and the language servers are saying to each other — useful when hover is silent or wrong. The daemon inherits the variable, so restart an already-running review to pick it up. Everything lands in the daemon log, `~/.crit/sessions/<key>.log` (`tail -f` it while the review runs; crit deletes it when the session ends): each server's stderr (discarded otherwise), every JSON-RPC frame in both directions with payloads cut to 1 KB, one summary line per request with its duration and outcome, and server lifecycle events — start, handshake, exit, restart, idle shutdown. Lines are tagged `lsp[go]` / `lsp[typescript]` / `lsp[python]`; `grep 'finished in'` gives a per-request timing view. Frames include the source text of files you hover, so don't paste the log somewhere public.
