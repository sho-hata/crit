...
- **Range / PR reviews** (`--range`, `--pr`) show file content at a fixed SHA, which can differ from what's on disk. Rather than answering from the working tree and silently mismatching the diff you're reading, crit anchors the language servers at a sparse git worktree checked out at that SHA — built on the first LSP request and torn down on the same idle timeout. Local git only; `--remote` and Sapling/JJ range reviews report `lsp_available: false`. The checkout contains only the languages actually present in the review — a Go-only review doesn't pay for tracked JS assets just because typescript-language-server is installed. Cap the checkout size with `lsp_worktree_max_mb` (default 500) — a commit whose estimated checkout exceeds it is skipped rather than checked out. For TypeScript, `node_modules` is not part of the checkout (it isn't tracked by git), so cross-package hover/definition degrades in range/PR focus while intra-repo symbols keep working. Python's `.venv` is untracked too, so crit reads it from your working tree: third-party types come from your *local* install, not the PR's dependencies (a bumped version shows the old API, a new dependency shows `Unknown`). Python hovers, and peeks into a local package, carry a one-line note saying so.
- **Python virtualenvs.** pyright doesn't find a `.venv` by itself: without help every third-party import is `Unknown` (no type on hover, definitions land on the `import` line) and nothing reports an error. crit points it at the site-packages of the `.venv/` or `venv/` nearest the file, which also covers per-service venvs in a monorepo. It passes a directory, never an interpreter, so crit never runs Python from the repository under review. Other setups are left to pyright: activate the venv before starting crit (an already-running review keeps its original `PATH`, so restart it) or add a `pyrightconfig.json` with `venvPath` and `venv`. Editable-install `.pth` files and a Python version other than the one on `PATH` aren't picked up from a `.venv`.
