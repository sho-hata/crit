...
Config keys: `port`, `host`, `no_open`, `quiet`, `output`, `author`, `base_branch`, `ignore_patterns`, `auto_viewed_patterns`, `default_markdown_view`, `agent_cmd`, `plan_approve_mode`, `cleanup_on_approve`, `notify_on_round_ready`, `disable_stats`, `no_update_check`, `no_integration_check`, `vcs`, `live_cookie`, `live_cookie_file`, `live_cdp_url`, `close_on_approve_after_ms`, `stale_review_days`, `lsp`, `lsp_worktree_max_mb`.
...
- `agent_cmd`, `plan_approve_mode`, `close_on_approve_after_ms`, and `stale_review_days` are **global config only**; project-level config cannot override (security — prevents malicious repos from hijacking the agent command, weakening Claude Code permissions, forcing a reviewer's tab to auto-close, or shortening review retention for every project)
