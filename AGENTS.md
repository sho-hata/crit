...
- `lsp` (default: `true`) — language-server features (hover, go-to-definition) for Go and TypeScript/JavaScript files in the review UI. Each language activates when its server (`gopls`, `typescript-language-server`) is on PATH. Mergeable from project config: the spawned binary names are fixed (PATH lookup, no path key), so a malicious repo cannot hijack the command.
