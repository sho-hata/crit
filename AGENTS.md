...
17. **LSP integration (Go + TypeScript/JavaScript + Python)** — `internal/lsp` speaks minimal LSP over stdio for hover / go-to-definition / find-references (`/api/lsp/*`, frontend `web/crit-lsp.js`). Supported servers live in the `internal/lsp/language.go` registry: `gopls` (`.go`) and `typescript-language-server --stdio` (`.ts/.tsx/.mts/.cts/.js/.jsx/.mjs/.cjs`) and `pyright-langserver --stdio` (`.py/.pyi`); each activates independently when its binary is on PATH, and `/api/config` exposes the covered extensions as `lsp_extensions` (the frontend hardcodes nothing). Lifecycle: **lazy start** (one server per language, spawned on first LSP request for that language, not daemon start) + **idle shutdown** (all killed after 3 min without requests) + killed on daemon shutdown. Each daemon owns its own manager, so N parallel worktree reviews only pin server processes for sessions actively being hovered. Positions are UTF-16 (LSP default = JS native — no conversion). **Debug logging**: `CRIT_LSP_DEBUG=1` (`lsp.DebugEnv`, read per call site via `debugEnabled()`; the daemon inherits it, so restart to apply) routes each server's stderr — otherwise `io.Discard`ed in `startServer` — plus every JSON-RPC frame (payload cut to `maxDebugPayload`), a per-request `finished in` summary, and lifecycle events through the standard logger into the daemon log `~/.crit/sessions/<key>.log`; new LSP code paths that fail silently should log through `debugf`, never unconditionally. **Both renderings of a code file are wired up** in `eligibleLineEl`: the diff view (`.diff-content` inside `.diff-line` / `.diff-split-side`) and document view (`.code-document .line-content` inside a single-line `.line-block`) — file mode (`crit some.ts`) never renders code as a diff, so binding to diff markup alone made hover/definition silently dead there. **Handshake settings** come from `Language.InitOptions`: TypeScript pins `tsserver.path` to the `node_modules/typescript` nearest the requested file, walking up to the workspace root — without it `typescript-language-server` exits at `initialize` in any monorepo whose dependency lives in a sub-package (`frontend/node_modules/typescript`) rather than at the repo root the workspace is anchored to. No install anywhere sends no options, leaving the server's own (global) resolution in charge. **Some servers take settings only through a `workspace/configuration` pull, not `initializationOptions`** — pyright reads `python.*` that way and ignores the latter. That is `Language.ConfigSettings` (answered by `Client.respondToServerRequest`, resolved with the same working-tree `depRoot` fallback as `InitOptions` via `Manager.handshake`); a language with none never has `capabilities.workspace.configuration` declared, so gopls and typescript-language-server see exactly the wire they always did. **pyright does not discover a `.venv`** — without help every third-party import is `Unknown` and nothing errors — so `pyConfigSettings` sends `python.analysis.extraPaths` = the nearest `.venv`/`venv` site-packages. **Never `python.pythonPath`**: it makes pyright execute that interpreter, and here it would come from the tree under review (a branch committing `.venv/bin/python` would run code just by being opened in a diff); a directory is safe, an interpreter is not. For the same reason the one `python3 -c` crit runs itself (`pythonEnvRoots`, for peek roots) uses `-I` — without it the daemon's cwd, the repo, is first on `sys.path` and a committed `sysconfig.py` would run. pyright queues requests behind its own analysis and reports no startup progress, so `Language.SkipReadyWait` skips `WaitReady` for it (the wait would just burn its 1s grace on the first hover). **The first request after a `didOpen` waits for the server's work-done progress to end** (`Client.WaitReady`, opted into via the `window.workDoneProgress` capability): a TypeScript server answers from a half-built project without flagging it — definitions resolve to the import statement itself, hovers come back bare — and unlike gopls's retryable `no views` there is no error to catch. Warm servers skip the wait. Definition/reference peeks only read files under repo root plus each installed language's extra roots from the registry (`Language.ExtraRoots`): GOROOT / GOMODCACHE for Go, the global `node_modules` (`npm root -g`) for TypeScript, the PATH `python3`'s stdlib + site-packages, the project `.venv` and pyright's bundled `typeshed-fallback` for Python (`ExtraRoots` receives the tree dependencies live in — the working tree under range focus, so `.venv` is found there) — locally-installed TS `node_modules` targets fall under repo root, globally-installed `typescript` libs (`lib.*.d.ts`) under the npm global root; there is deliberately no general file-read endpoint. **Range/PR focus** (local git only, not `--remote`/Sapling/JJ) roots LSP at a sparse worktree checked out at `Focus.HeadSHA` instead of the working tree, since the review pane shows HeadSHA content that can differ from disk — see `syncLSPRoot` in `internal/server/lsp_handlers.go`. The sparse pattern set is the union of patterns for the languages that both cover a session file and have their server installed (content-based — an installed language absent from the review doesn't inflate the checkout or its size estimate); `node_modules` is untracked so TS cross-package resolution degrades in range focus — the server still starts, because `Manager.depRoot` falls back to the working tree at the same repo-relative path when the worktree has no `typescript` to pin (the same fallback finds a Python `.venv`, so pyright resolves third-party imports in range focus). **That borrowed environment is the reviewer's local one, not the reviewed SHA's** (a bumped dependency shows the old API, a new one shows `Unknown`, silently), so under range focus the server marks such answers `local_env: true` — the hover response for a language with `Language.LocalEnv`, and a definition/reference location under a `PeekRoot.LocalEnv` root (Python site-packages; not the stdlib) — and the frontend renders one muted line (`.lsp-tooltip-note`, `.lsp-peek-note`), never a toast. Working-tree focus gets none. Built lazily on the first LSP request against a given commit, size-guarded by `lsp_worktree_max_mb` (default 500), and torn down on the same idle timeout as the servers, on a focus switch to a different SHA (or a pattern-set change, e.g. a server installed mid-session), and on daemon shutdown; a worktree orphaned by a crashed daemon is self-healed on the next daemon start.
...
Config keys: `port`, `host`, `no_open`, `quiet`, `output`, `author`, `base_branch`, `ignore_patterns`, `auto_viewed_patterns`, `agent_cmd`, `plan_approve_mode`, `cleanup_on_approve`, `notify_on_round_ready`, `disable_stats`, `no_update_check`, `no_integration_check`, `vcs`, `live_cookie`, `live_cookie_file`, `live_cdp_url`, `close_on_approve_after_ms`,`lsp`, `lsp_worktree_max_mb`.
...
- `lsp` (default: `true`) — language-server features (hover, go-to-definition) for Go, TypeScript/JavaScript and Python files in the review UI. Each language activates when its server (`gopls`, `typescript-language-server`, `pyright-langserver`) is on PATH. Mergeable from project config: the spawned binary names are fixed (PATH lookup, no path key), so a malicious repo cannot hijack the command.
- `lsp_worktree_max_mb` (default: `500`) — size cap (MB) for the sparse git worktree LSP builds for range/PR focus (see item 17 above). A commit whose checkout estimate exceeds this is skipped rather than checked out.
...
- `GET  /api/lsp/hover?path=X&line=N&char=M` — language-server hover markdown for a position in a covered file (Go / TS / JS / Python; line 1-based, char 0-based UTF-16); adds `local_env: true` under range focus for a language whose third-party answers come from the local environment
- `GET  /api/lsp/definition?path=X&line=N&char=M` — definition locations, each with an inline peek when the target is under repo root or an installed language's extra roots (GOROOT / GOMODCACHE / global `node_modules`) (whole file ≤2000 lines; ±100-line window + `peek_truncated` above that). `path` is repo-relative from the review pane, or absolute for chained jumps from the peek popup — absolute paths are accepted ONLY under those same roots (403 otherwise); never widen this into a general file API.
